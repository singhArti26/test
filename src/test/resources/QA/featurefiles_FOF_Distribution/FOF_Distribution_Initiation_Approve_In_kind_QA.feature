...
###############################################################################################################################################################################
@Reg_Areteans__AlterDomusFOF_QA @FOF_Dis2QA @RegressionRunQA
Feature: Verify that CSL1 or CL2 initiate case with distribution type is in kind and reslove the case by CSL2
  ...
  Scenario Outline: Verify that user "<Username>" initiate case with "<Distritype>" with required fields
    ...
    Examples: 
      ...
      | ClientServices002@alterdomus.com | Welcome@123 | FOF Distribution | ATKemp Hickinbottom Co. |    30000 | Foundation Protyping Inc. |          20000 | In kind    | Amortization: Insurance | Amortization: Management Fees | \\attachments\\document1.pdf | L2 – FoF Distribution Initiation |
  ...
  Scenario Outline: Verify that user "<Username>" approves case and Audit History for FOF Distribution with required fields
    ...
    Examples: 
      ...
      | Clientservices001@alterdomus.com | Welcome@123 | L1 – Cash Collection and Recon |
