...
###############################################################################################################################################################################
@Reg_Areteans__AlterDomusF0F_QA @FOFDis1_QA @RegressionRunQA
Feature: Verify that CSL1 or CL2 initiate with cancel request and Reject the cancel request then reslove the case
  ...
  Scenario Outline: Verify that user "<Username>" initiate case with cancel request option with required fields
    ...
    Examples: 
      ...
      | ClientServices002@alterdomus.com | Welcome@123 | FOF Distribution | ATOgilvie Midas LLC |    30000 | Foundation Protyping Inc. |          20000 | Cash       | Asset: Inventory | Amortization: Management Fees | \\attachments\\document1.pdf | Auto reject       | Cancel Request | L2 – FoF Distribution Initiation |
  ...
  Scenario Outline: Verify that user "<Username>" Reject cancelled case by CSL2
    ...
    Examples: 
      ...
      | Clientservices001@alterdomus.com | Welcome@123 | L2 – FoF Distribution Initiation | CSL2 enter the reject case |

  @alterDomus_Regression_set1_Tos_QA @cancelrequest_QA
  Scenario Outline: Verify that user "<Username>" assign Priority and Approve case by CSL2
    ...
    Examples: 
      ...
      | Clientservices001@alterdomus.com | Welcome@123 | Assign Priority | L1 – Cash Collection and Recon |

  @alterDomus_Regression_set1_Tos_QA @cancelrequest_1_QA
  Scenario Outline: Verify that user "<Username>" validate review paged and Accounting platform
    ...
    Examples: 
      ...
      | Clientservices002@alterdomus.com | Welcome@123 | Priority       | L2 – Cash Coll. and Recon Review |
