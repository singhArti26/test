...
#############################################################################################################
@RegressionDirectInvestmentSaleProcess_QA @RegressionDISScenario2_QA @RegressionRunQA
Feature: Verify that client services team initiates the case using random fund(use template) - submits "cancel request" - L2 approves the cancel request
  ...
  Scenario Outline: CS L1 initiates the case - performs L1 sale of investment review - submits the case requesting cancellation review
    ...
    Examples: 
      ...
      | Clientservices002@alterdomus.com | Welcome@123 | Yes                      | Executed      | testing    | \\DirectInvestmentResources\\sample.docx | L1 - Sale of Inv. Initiation | invalid    | Initiate Cancellation | Client Services 002 |
  ...
  Scenario Outline: CS L2 accepts the cancellation request sent by CS L1 --> Case status changes to "<Status_Cancel>"
    ...
    Examples: 
      ...
      | Clientservices001@alterdomus.com | Welcome@123 | Yes                      | Executed      | testing    | \\DirectInvestmentResources\\sample.docx | L1 - Sale of Inv. Initiation | Resolved-Cancelled | invalid    | Review Cancel Request Decision | Client Services 001 |
