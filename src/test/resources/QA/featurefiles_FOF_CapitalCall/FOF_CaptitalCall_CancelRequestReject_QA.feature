...
#############################################################################################################
@RegressionFOFCapital_QA @FOFCapitalCall_3QA @RegressionRunQA
Feature: Verify that CS L1 initiates the case and submits cancel request, CS L2 rejects cancel request
  ...
  Scenario Outline: Verify that user "<Username>" initiate case for FOF Capital Call with required fields
    ...
    Examples: 
      ...
      | Clientservices002@alterdomus.com | Welcome@123 | FOF Capital Call | ATOgilvie Midas LLC | TATA             | \\DirectInvestmentResources\\Invoice.pdf | 15000 | USD      | ICICI 1         | Glaswegian Slone Inc. | Aire Irvine (Units) |         876567 |     6765 |         7658 | Asset: Inventory  |   15000 | Testing    | Testing    | Testing    |
  ...
  Scenario Outline: "<Username>" accepts the case
    ...
    And User enters Reason for rejection "<Cancel_Rsn>"
    And click on Submit button
    And wait for success message
    ...
    Examples: 
      | Username                         | Password    | Intl_cmnts | Action_name                    | Action_By           | Cancel_Rsn |
      | Clientservices001@alterdomus.com | Welcome@123 | Testing    | Review Cancel Request Decision | Client Services 001 | invalid    |
