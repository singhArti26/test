...
#############################################################################################################
@RegressionFOFCapital @FOFCapitalBulk6 @RegressionFinal @ArtiTag
Feature: Verify that CS L1 initiates the case using template and CS L2 picks the case

  @FOFCapital_useTemplte_001
  Scenario Outline: Verify that user "<Username>" initiate case for FOF Capital Call with required fields
    ...
    Examples: 
      ...
      | ATClientservices002@alterdomus.com | Welcome@123 | FOF Capital Call | ATOgilvie Midas LLC | TATA             | \\DirectInvestmentResources\\Invoice.pdf | 15000 | USD      |         876567 |     6765 |         7658 | Asset: Inventory  |   15000 | Testing    | Testing    | L1 Fund Of Fund Capital Call Enrichment | Client Services 002 AT | FoF Capital Call Initiation |

  @FOFCapital_useTemplte_002
