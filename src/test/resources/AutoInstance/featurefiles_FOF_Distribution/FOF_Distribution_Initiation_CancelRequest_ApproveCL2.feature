...
###############################################################################################################################################################################
@Reg_Areteans__AlterDomusF0F @RegressionFinal1 @FOF_Scenario4_AutomationIns @FOFDisBulk
Feature: Verify that CSL1 or CL2 initiate and cancel the FOF distribution case by CSL2
  ...
  Scenario Outline: Verify that user "<Username>" initiate case with required fields and cancel the case using Cancel Request
    ...
    Examples: 
      ...
      | ATClientServices002@alterdomus.com | Welcome@123 | FOF Distribution | ATOgilvie Midas LLC |    30000 | Foundation Protyping Inc. |          20000 | Cash       | Amortization: Insurance | Amortization: Management Fees | \\attachments\\document1.pdf | Cancel Request | Auto cancel       | L2 – FoF Distribution Initiation |
  ...
  Scenario Outline: Verify that user "<Username>" approve pending cancelled case and status for FOF by CSL2
    ...
    Examples: 
      ...
      | ATClientservices001@alterdomus.com | Welcome@123 | Resolved-Cancelled |
