...
###############################################################################################################################################################################
@Reg_Areteans__AlterDomusF0F @RegressionFinal1 @FOF_Scenario3_AutomationIns @FOFDisBulk
Feature: Verify that CSL1 or CL2 initiate and transfer to another CSL2 then resloves the FOF distribution case

  @alterDomus_Regression_set1 @Demo_FoFDis
  Scenario Outline: Verify that user "<Username>" initiate case for with required fields
    ...
    Examples: 
      ...
      | ATClientServices002@alterdomus.com | Welcome@123 | FOF Distribution | ATRickwood Handbook Co. |    30000 | Foundation Protyping Inc. |          20000 | Cash       | Amortization: Insurance | Amortization: Management Fees | \\attachments\\document1.pdf | L2 – FoF Distribution Initiation |

  @alterDomus_Regression_set1 @Demo_FoFDis @testonly
  Scenario Outline: Verify that user "<Username>" transfer the case to "<otherCSL2>" for FOF Distribution with required fields
    ...
    Examples: 
      ...
      | ATClientservices001@alterdomus.com | ATClientservices003@alterdomus.com | Welcome@123 | Transfer Assignment | L2 – FoF Distribution Initiation |
  ...
  Scenario Outline: Verify that user "<Username>" Approve case for FOF Distribution with required fields
    ...
    Examples: 
      ...
      | ATClientservices003@alterdomus.com | Welcome@123 | L1 – Cash Collection and Recon |

  @alterDomus_Regression_set1 @Demo_FoFDis_2
  ...
  Scenario Outline: Verify that user "<Username>" reslove case for case transfered by Clientservices001
    ...
    Examples: 
      | Username                           | Password    | status             |
      | ATClientservices003@alterdomus.com | Welcome@123 | Resolved-Completed |
